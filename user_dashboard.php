...
// Dapatkan ID Cabang dan Hak Akses Rekening
...
if (isset($_POST['simpan_transaksi']) && $status_shift_aktif && !$cabang_dibekukan) {
    $shift_data_aktif = $q_shift_aktif->fetch_assoc();
    $shift_id = $shift_data_aktif['id'];
...
// PERHITUNGAN SALDO FISIK & DIGITAL DINAMIS
$saldo_fisik_sekarang = 0;
$total_saldo_digital = 0;
$admin_hari_ini = 0;

if ($status_shift_aktif) {
    $q_shift_ulang = $conn->query("SELECT * FROM shifts WHERE user_id = '$user_id' AND tanggal = '$tanggal_hari_ini' AND status = 'aktif' ORDER BY id DESC LIMIT 1");
    $shift_data = $q_shift_ulang->fetch_assoc();
    $sid = $shift_data['id'];

    $q_in_cash = $conn->query("SELECT SUM(nominal) as total FROM transactions WHERE shift_id = '$sid' AND jenis_transaksi NOT IN ('Tarik Tunai', 'Tukar Uang')");
    $uang_masuk = $q_in_cash->fetch_assoc()['total'] ?? 0;

    $q_keluar = $conn->query("SELECT SUM(nominal) as total FROM transactions WHERE shift_id = '$sid' AND jenis_transaksi = 'Tarik Tunai'");
...
$grafik_tanggal = []; $grafik_pendapatan = [];
for ($i = 6; $i >= 0; $i--) {
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-12 col-sm-6 col-xl-3">
                <div class="metric-card bg-mbanking">
                    <p class="mb-1 fw-semibold small opacity-75 text-uppercase tracking-wide">Saldo <?= $jumlah_rek ?> Rek (Digital)</p>
...
<script>
    ...
    function sesuaikanForm(jenis) {
        ...
        if (jenis === 'Buka Rekening Baru') {
            ...
            htmlFields = `<div class="mb-3"><label class="form-label text-muted small fw-bold text-uppercase">Nama Nasabah / NIK</label><input type="text" id="field_nama_nasabah" class="form-control form-control-sm trigger-compile" placeholder="Nama Lengkap / NIK" required style="border-radius: 10px;"></div>`;
    ...
    function compileKeterangan() {
        ...
        let res = "";
        if (jenis === 'Buka Rekening Baru') res = "Nasabah: " + (document.getElementById('field_nama_nasabah') ? document.getElementById('field_nama_nasabah').value : '');
